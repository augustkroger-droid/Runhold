...
add column if not exists min_object_count integer not null default 0;

alter table public.player_map_sectors
add column if not exists max_object_count integer not null default 0;
...
create index if not exists player_map_objects_sector_live_idx
on public.player_map_objects (user_id, sector_key, collected_at, expires_at);
  ...
  if input_walkable_candidates is not null
    ...
    loop
      ...
      if candidate_sector.candidate_count >= 16 then
        initial_min_objects := 3;
        initial_max_objects := 6;
        spacing_m := 75.0;
      ...
      values (
        ...
        now()
          + (
            (2 * 3600 + floor(random() * 4 * 3600)::integer)
            * interval '1 second'
          )
      )
      on conflict on constraint player_map_sectors_pkey do update
      set
        max_object_count = greatest(
          public.player_map_sectors.max_object_count,
          excluded.max_object_count
        ),
        min_object_count = case
      ...
      computed_next_spawn_at := now()
        + (
          (3 * 3600 + floor(random() * 4 * 3600)::integer)
          * interval '1 second'
        );
