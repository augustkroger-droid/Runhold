...
alter table public.player_map_sectors
add column if not exists last_spawned_at timestamptz;

create index if not exists player_map_objects_expiry_idx
on public.player_map_objects (user_id, collected_at, expires_at);
...
declare
  ...
  base_sector_capacity integer := 3;
  max_sector_capacity integer := 6;
  spawn_rate_per_scan integer := 2;
  spawn_cooldown interval := interval '6 hours';
  object_lifetime interval := interval '72 hours';
  ...
  if not active_expedition_exists then
    ...
    where
      ...
      and (
        ...
        or (objects.collected_at is not null and objects.collected_at <= now() - interval '1 hour')
      );
  end if;
  ...
  loop
    ...
    sector_capacity := least(
      ...
      base_sector_capacity
        + floor((least(7.0, greatest(0.0, sector_age_days)) / 7.0) * base_sector_capacity)::integer
    );
    ...
    if current_live_objects > 0
      and sector_state.last_spawned_at is not null
      and sector_state.last_spawned_at > now() - spawn_cooldown then
      continue;
    end if;

    objects_to_spawn := least(spawn_rate_per_scan, sector_capacity - current_live_objects);

    if current_live_objects = 0 then
